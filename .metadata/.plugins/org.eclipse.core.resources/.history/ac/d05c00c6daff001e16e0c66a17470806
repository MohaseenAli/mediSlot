...
<body>
	...
	<div class="container">
		<div class="form-container">
			...
			%>
			<form id="appointmentForm" method="post" action="bookedAppointment">
				...
				<div class="form-group">
					<label for="age">Age</label> <input type="number"
						class="form-control" id="age" name="age"  min="1" max="110"
				...
				<div class="form-group">
					<label for="doctor">Doctor</label> <select class="form-control"
						id="doctor" name="doctor" onchange="loadAppointmentDates()"
						required>
				...
				<div class="form-group">
					<label for="preferredDate">Select Date for the Appointment</label>
					<select class="form-control" id="preferredDate"
						name="preferredDate" onchange="loadAppointmentTimes()" required>
						<option value="">Select Doctor First</option>
					</select>
				</div>

				<div class="form-group">
					<label for="preferredTime">Select Time for the Appointment</label>
    ...
    function loadAppointmentDates() {
        ...
        if (doctorId) {
            // Send an AJAX request to fetch available appointment times
            fetch('getAvailableDates?doctorId=' + doctorId)
            .then(response => response.json())
            .then(data => {
            	
                var dateSelect = document.getElementById('preferredDate');
            ...
            // Clear the time options if no doctor is selected
            document.getElementById('preferredDate').innerHTML = '<option value="">Select Doctor First</option>';
        }
    }
</script>
    ...
    function loadAppointmentTimes() {
        ...
        if (doctorId && preferredDate) {
            // Send an AJAX request to fetch available appointment times
            fetch('getAvailableTimes?doctorId=' + doctorId+'&preferredDate='+preferredDate)
            .then(response => response.json())
            .then(data => {
            ...
            // Clear the time options if no doctor is selected
            document.getElementById('preferredTime').innerHTML = '<option value="">Select Doctor And Date First</option>';
        }
    }
</script>
